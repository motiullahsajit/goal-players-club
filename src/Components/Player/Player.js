            ...
            <div className="row g-0">
                ...
                <div className="col-md-8">
                    <div className="card-body">
                        ...
                        <h5>Email: {email}</h5>
                        <p>{info}</p>
                        <h4><span style={backgroundColor} className="badge rounded-pill text-success">Annual Salary: {salary} $</span></h4>
                        <button disabled={isClicked} onClick={() => {
                            handleAddPlayer(props.player)
                            setIsClicked(true)
                        }} className='btn text-success m-1' style={backgroundColor}><FontAwesomeIcon icon={faUserPlus} className='me-1' />{isClicked ? "Added To Club" : "Add To Club"}
                        </button>
                    </div>
                </div>
            </div>
